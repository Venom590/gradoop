...
import org.gradoop.flink.algorithms.fsm.cross_level.functions.CollapseDimensionProperties;
import org.gradoop.flink.algorithms.fsm.cross_level.functions.ExpandDimensionProperties;
 ...
 */
public class CrossLevelDIMSpan implements UnaryCollectionToCollectionOperator {
   ...
   */
  public CrossLevelDIMSpan(float minSupport) {
   ...
   */
  public CrossLevelDIMSpan(DIMSpanConfig fsmConfig) {
    dimSpan = new DIMSpan(fsmConfig);
  }
  ...
  public GraphCollection execute(GraphCollection collection) {
    ...
    DataSet<LabeledGraphStringString> input = collection
      .toTransactions()
      .getTransactions()
      .map(new ExpandDimensionProperties())
      .map(new EPGMGraphTransactionToLabeledGraph());

    // run DIMSpan
    DataSet<GraphTransaction> output = dimSpan
      .execute(input)
      .flatMap(new CollapseDimensionProperties());
